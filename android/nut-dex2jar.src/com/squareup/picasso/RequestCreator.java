...
public class RequestCreator
{
  ...
  public void fetch(Callback paramCallback)
  {
    ...
    return;
    label136: paramCallback = new FetchAction(this.picasso, localRequest, this.memoryPolicy, this.networkPolicy, this.tag, str, paramCallback);
  ...
  public RequestCreator memoryPolicy(MemoryPolicy paramMemoryPolicy, MemoryPolicy[] paramArrayOfMemoryPolicy)
  {
    ...
    {
      int j = paramArrayOfMemoryPolicy.length;
      int i = 0;
      while (i < j)
      {
        paramMemoryPolicy = paramArrayOfMemoryPolicy[i];
        if (paramMemoryPolicy == null)
          throw new IllegalArgumentException("Memory policy cannot be null.");
  ...
  public RequestCreator networkPolicy(NetworkPolicy paramNetworkPolicy, NetworkPolicy[] paramArrayOfNetworkPolicy)
  {
    ...
    if (paramArrayOfNetworkPolicy.length > 0)
    {
      ...
      while (i < j)
      {
        paramNetworkPolicy = paramArrayOfNetworkPolicy[i];
        if (paramNetworkPolicy == null)
          throw new IllegalArgumentException("Network policy cannot be null.");
